...
\maketitle
% --- Pas de ligne de copyright ISBN : ce n'est pas une omission, c'est la regle d'ICIP.
% Le \IEEEpubid{XXX-X-XXXX-XXXX-X/26/...} qui se trouvait ici venait du gabarit generique
% IEEEtran, pas d'ICIP. Mesure (2026-09-09) :
%   - Le kit LaTeX officiel ICIP 2026 (spconf.sty + Template.tex, civit.fi/.../LaTeX_template.zip)
%     definit la macro \copyrightnotice mais ne l'appelle JAMAIS : 0 occurrence dans Template.tex,
%     et spconf.sty:217 fait \let\@copyrightnotice\relax — donc vide par defaut.
%   - Le paper kit ICIP (2026 et 2025) n'exige une notice de copyright que si l'auteur publie
%     lui-meme une copie sur Internet, et cette notice-la est du texte, sans ISBN.
%   - L'ISBN appartient au volume des actes, pas au papier : ICIP 2025 avait 979-8-3315-2380-0
%     (conference) et 979-8-3315-7799-5 (workshops). Il n'est jamais transmis aux auteurs.
% Conclusion : aucune chaine a attendre. La ligne est retiree plutot que remplie.
% ------------------------------------------------------------
\begin{abstract}
...
% \IEEEpubidadjcol retire avec le \IEEEpubid : sans boite de pubid, il raccourcirait
...
workflows, or lightweight publisher scripts. On the image quality axis, LSB embedding at $k{=}1$ yields $51.2$\,dB. For context, \cite{omniguard} evaluates both systems on its own benchmark and reports, in Table~3, a copyright-only setting ($42.15$\,dB for EditGuard, $42.33$\,dB for OmniGuard) and a joint copyright-plus-localization setting ($37.53$ and $41.78$\,dB respectively); the latter is the closer analogue to PBC, which also carries localization. These are third-party measurements taken under a different dataset and at a different payload --- $64$ and $100$ bits respectively, against the $48$-bit chain hash PBC writes per block --- so they are not directly comparable to ours. We read the gap as a consequence of divergent design points rather than a like-for-like advantage: neural schemes deliberately trade fidelity for survival across the lossy channels that PBC does not target.
...
\bibitem{fang2020blockchain}
  ...
  Advances in Intelligent Systems and Computing, vol.~1117,
  pp.~420--428, Springer, 2020.

\bibitem{editguard}
  X.~Zhang, R.~Li, J.~Yu, Y.~Xu, W.~Li, and J.~Zhang,
  ``EditGuard: Versatile Image Watermarking for Tamper Localization
  and Copyright Protection,''
  in \emph{Proc. IEEE/CVF CVPR}, pp.~11964--11974, 2024.
