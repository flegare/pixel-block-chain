...
workflows, or lightweight publisher scripts. On the image quality axis, LSB embedding at $k{=}1$ yields $51.2$\,dB. The closest published comparison point is the joint copyright-plus-localization setting of~\cite[Table~3]{omniguard}, which matches our own scope since PBC likewise carries localization: there EditGuard reports $37.53$\,dB and OmniGuard $41.78$\,dB. We read this gap as a consequence of divergent design points rather than a like-for-like advantage: neural schemes deliberately trade fidelity for survival across the lossy channels that PBC does not target.
